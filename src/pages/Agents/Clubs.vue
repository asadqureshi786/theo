  ...
  <div class="clubs_page">
    ...
    <div class="page_header">
      ...
      <div class="r_side">
        ...
        <div class="form-group">
          <div class="d_select">
            <Select v-model="selectedLeague" @change="fetchClub" editable :options="leagues" optionLabel="name" placeholder="League"
              class="w-full" />
          </div>
        </div>
...
export default {
  ...
  methods: {
    async fetchCulbs() {
      try {
        const response = await axios.get(this.$baseURL + 'theo/api/agent/clubs', {
          headers: {
    ...
    async selectLeague() {
      ...
      if(this.selectedCountry?.id){
        try {
                          this.allClubs = [];
        const response = await axios.get(this.$baseURL + `theo/api/agent/clubs/${this.countryID}/leagues`, {
          headers: {
            'Accept': 'application/json',
            Authorization: `Bearer ${this.token}`,
          },
